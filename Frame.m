...
for a = 1 : length(ModTypeOptions)
    for b = 1 : length(ModOrder)
        ...
        for i = 1 : SymbolNum
            ...
            OFDMSymbol(i,:) = sum(Subcar);
            OFDMSymbol(i,:) = OFDMSymbol(i,:) ./ max(abs(OFDMSymbol(i,:)));
        end
        OFDMSymbol = [zeros(SymbolNum,1) OFDMSymbol];
        for  z = 1 : length(NoiseMagnitude)
            tic
            Noise = NoiseMagnitude(z) .* rand(SymbolNum, length(OFDMSymbol(1,:)));
            ...
            for i = 1 : SymbolNum
                ...
                parfor j = 1 : SubcarNum
                    IRX = SymbolsRX(i,1:end-1) .* real(exp(1i*2*pi*(LowFreq + SubcarSpace(j)/SymbolT).*t));
                    QRX = SymbolsRX(i,1:end-1) .* (-imag(exp(1i*2*pi*(LowFreq + SubcarSpace(j)/SymbolT).*t)));
        ...
        Filename = ['BER_Frame' sprintf( '%02d', M ) ModType '.mat'];
